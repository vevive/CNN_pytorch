...
class Resnet(nn.Module):
    def __init__(self,depth,num_classes,block_name='BasicBlock'):
        ...
        if block_name='BasicBlock':
            assert (depth-2)%6==0,'depth should be 6n+2, e.g 20,32,44,56,110,1199'
            n=(depth-2)//6
            block=BasicBlock
        ...
        self.relu=nn.ReLU(inplace=True)
        self.stage_1=self._nake_layer(block,16,n)
    ...
    def forward(self,x):
        ...
        x=self.stage_1(x)    #32*32
        x=self.stage_2(x)    #16*16
        x=self.stage_3(x)    #8*8
